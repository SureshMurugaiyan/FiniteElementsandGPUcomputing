...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
void DivCPU(double* Dn, double* Phi, double* U, double* V, int row, int col,double delX,double delY){
...
__global__ void Divergence(double* Dn, double* U, double* V,int row, int col, double delX, double delY){
 ...
 void DivergenceCPU(double* Dn, double* U, double* V,int row, int col, double delX, double delY){
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
void LaplacianCPU(double* Ln, double *Phi, int row, int col, double delX, double delY){
for(int i = 1; i<(row-1); i++){
 for(int j =1; j<(col-1); j++){
   int k = i*col+j;
   double PhiP = Phi[k];
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
// Gradient Computation using Finite Difference                             !
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
__global__ void gradient(double* gradxPhi,double* gradyPhi,double* Phi,
  ...
  int n  = (row-1)*(col-1);
   ...
   gradxPhi[k] = (Phie-Phiw)/(2*delX);
   gradyPhi[k] = (Phin-Phis)/(2*delY);
    
  }
}

void gradientCPU(double* gradxPhi,double* gradyPhi,double* Phi,
                        int row, int col, double delX, double delY){
for(int i = 1; i<(row-1); ++i){
 for(int j =1; j<(col-1); ++j){
